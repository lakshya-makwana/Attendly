...
router = APIRouter(prefix="/sites", tags=["Sites"], dependencies=[Depends(get_current_admin)])

@router.get("", response_model=List[SiteResponse])
def get_sites(active_only: bool = False, db: Session = Depends(get_db)):
    query = db.query(Site)
    if active_only:
...
def update_site(site_id: int, site_in: SiteUpdate, db: Session = Depends(get_db)):
    ...
    # In-place edit preserves site_id and all historical attendance/analytics associations
...
def delete_site(site_id: int, db: Session = Depends(get_db)):
    ...
    attendance_count = db.query(Attendance).filter(Attendance.site_id == site_id).count()
